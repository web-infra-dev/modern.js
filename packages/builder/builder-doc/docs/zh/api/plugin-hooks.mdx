...
- `modifyBundlerChain` (experimental)：通过 chain api 修改 webpack / Rspack 配置。
...
通过 Builder 提供的 bundler chain 来修改 webpack / Rspack 的配置。类 webpack chain，该 hook 支持在使用 webpack-provider / rspack-provider 时调用。
