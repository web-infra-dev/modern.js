...
近年来，前端工具链的发展趋势是使用 Rust 等编程语言重新实现 —— 以获得更好的性能表现。社区中出现 [SWC](https://swc.rs/)、[esbuild](https://github.com/evanw/esbuild)、[Rspack](/guide/glossary.html#rspack) 等高性能工具，并且应用领域在逐步扩宽。但这些 Rust 工具与 JavaScript 工具之间存在较多差异，包括功能不完整、配置不一致等，导致使用者需要承担一定的迁移成本。
...
除了 webpack 打包，Builder 也支持一键切换到**基于 Rust 的高性能打包工具 —— Rspack**。Rspack 是字节跳动 Web Infra 团队研发的 Rust Bundler，核心架构对齐 webpack 实现，并对社区常用的构建能力做了开箱即用的支持。从长期来看，Rspack 会对齐 webpack 的主要 API，兼容主流的 webpack loader 与 plugin 生态，以保证开发者可以平滑地从 webpack 迁移到 Rspack。

请参考 [使用 Rspack](/guide/advanced/rspack-start) 来切换到 Rspack 构建模式。
