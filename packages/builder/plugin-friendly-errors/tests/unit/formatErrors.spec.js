...
const simple = errors =>
  errors.filter(error => !error.type).map(e => e.message);

const allCaps = errors =>
  errors
    .filter(error => error.type === 'other')
    .map(e => e.message.toUpperCase());

const notFound = errors =>
  errors.filter(error => error.type === 'not-found').map(() => 'Not found');
