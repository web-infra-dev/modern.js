...
The bundler chain is a subset of the webpack chain which the api inside is already Rspack compatible.

Modify config of webpack or Rspack via the bundler chain api, this hook is supported to be called when using webpack-provider / rspack-provider.
