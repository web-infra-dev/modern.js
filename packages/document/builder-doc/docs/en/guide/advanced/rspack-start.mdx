...
:::tip
When using component libraries such as [Arco Design](https://www.npmjs.com/package/@arco-design/web-react), you should avoid introducing less styles in full. You can instead import on demand, or directly import css styles.
:::
...
By default, when using [Ant Design](https://www.npmjs.com/package/antd) or [Arco Design](https://www.npmjs.com/package/@arco-design/web-react) component library, the on-demand import function will be automatically enabled, and the default configuration can refer to [source.transformImport](/api/config-source.html#sourcetransformimport).

We recommend that when using other component libraries, also configure on-demand import through [source.transformImport](/api/config-source.html#sourcetransformimport).
