...
Builder uses Rspack [builtin:swc-loader](https://www.rspack.dev/guide/loader.html#builtinswc-loader) for code translation. If you have custom configuration requirements for builtin:swc-loader, you can refer to the following code:
...
:::tip
The new version of Builder has switched from Rspack's built-in full compilation to `builtin:swc-loader` compilation.

The built-in compilation capability of Rspack is still used in Modern.js. All files will be compiled by default, [source.exclude](/api/config-source.html#sourceexclude) and [output.polyfill.usage](/api/config-output.html#usage) configuration are not supported.

If you want to switch to `builtin:swc-loader` compilation, you can refer to the following modifications:
