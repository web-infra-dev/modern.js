...
However, when configuration options or plugins that rely on babel are used in your project, babel-loader will be additionally added to transpile files:
...
TypeScript type checking is enabled by default when the Builder builds. The time-consuming of type checking in larger projects may far exceed the time-consuming of building Rspack itself. When building in production, this behavior may slow down the build to some extent.
...
Currently, Less files are processed using the JavaScript version of less-loader, and a Rust implementation version is not yet available.

##### Large number of SVG files

Builder supports using SVG resources as React components by default. If you are sure that all SVG resources in the project are not used as React components, you can set [output.disableSvgr](/api/config-output.html#outputdisablesvgr) to true to disable this behavior to improve build performance.
