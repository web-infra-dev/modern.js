...
:::tip
Builder provides the [source.transformImport](/api/config-source.html#sourcetransformimport.html) config, so you don't need to configure `extensions.pluginImport` manually.
:::
