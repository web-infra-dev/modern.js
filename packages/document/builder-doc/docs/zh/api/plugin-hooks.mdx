...
Bundler chain 是 webpack chain 的一个子集，里面的 api 已经做了 Rspack 兼容。

通过 Builder 提供的 bundler chain 来修改 webpack / Rspack 的配置。该 hook 支持在使用 webpack-provider / rspack-provider 时调用。
