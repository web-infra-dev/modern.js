...
- [output.enableLatestDecorators](/api/config-output.html#outputenablelatestdecorators)
...
Builder 使用 Rspack [builtin:swc-loader](https://www.rspack.dev/guide/loader.html#builtinswc-loader) 进行代码转译，如果对 builtin:swc-loader 有自定义配置的需求，可参考以下代码：
...
:::tip
Builder 新版本已经从 Rspack 内置的全量编译切换到 `builtin:swc-loader` 编译。

在 Modern.js 中仍使用 Rspack 内置的编译能力，默认所有文件都会经过编译，不支持 [source.exclude](/api/config-source.html#sourceexclude)、[output.polyfill.usage](/api/config-output.html#usage) 配置项。

如果希望切换到 `builtin:swc-loader` 编译，可参考以下修改：
