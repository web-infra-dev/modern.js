...
Builder 支持通过 [bundler-chain](/api/config-tools.html#toolsbundlerchain) 来修改 Rspack config。通过 bundler chain 修改的配置，在 webpack 和 Rspack 构建时均可生效。

:::tip
bundler-chain 只为 Rspack & webpack 配置提供一致的 api 修改方式，实际 plugin / loader 是否可用，需要看 Rspack  / webpack 实际支持情况。
:::
