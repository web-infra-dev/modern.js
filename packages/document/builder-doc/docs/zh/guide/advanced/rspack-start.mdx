...
:::tip
在使用 [Arco Design](https://www.npmjs.com/package/@arco-design/web-react) 等组件库时，应当避免全量引入 less 样式。可以改为按需引入，或直接引入 css 样式。
:::
...
默认情况下，在使用 [Ant Design](https://www.npmjs.com/package/antd) 或 [Arco Design](https://www.npmjs.com/package/@arco-design/web-react) 组件库时，会自动开启按需引入功能，默认配置可参考 [source.transformImport](/api/config-source.html#sourcetransformimport)。

我们推荐，在使用其他组件库时，也通过 [source.transformImport](/api/config-source.html#sourcetransformimport) 配置按需引入。
