...
在使用 Rspack 构建时，babel-loader 默认不会被启用。但在项目中使用到依赖 babel 实现的配置项或插件时，会额外添加 babel-loader 进行文件转译：
...
可通过开启[调试模式](/guide/debug/debug-mode.html)，查看最终生成的 Rspack 配置中是否存在 babel 插件。

##### 开启 TypeScript 类型检查

Builder 构建时会默认开启 TypeScript 类型检查。较大项目中类型检查的耗时可能远超过 Rspack 本身的构建耗时，在生产环境构建时，此行为会在一定程度上拖慢构建。

此时，可通过 [output.disableTsChecker](/api/config-output.html#outputdisabletschecker) 关闭类型检查，以查看排除类型检查后 Rspack 性能是否符合预期。

##### 大量 Less 文件

目前 Less 文件使用 js 版本的 less-loader 进行处理，还未提供 rust 实现版本。

##### 大量 SVG 文件

Builder 默认支持将 SVG 资源当成 React 组件使用。如果你确定项目内的所有 SVG 资源都没有当成 React 组件使用，可以通过将 [output.disableSvgr](/api/config-output.html#outputdisablesvgr) 设置为 true 来关闭此行为，以提升构建性能。
