...
- Type: `{ searchHook: string }`

You can add search runtime hooks logic through the `searchHook` parameter, for example:
...
export default defineConfig({
  doc: {
    search: {
      searchHook: path.join(__dirname, 'searchHook.ts'),
    },
  },
  plugins: [docTools()],
});
...
In `searchHook.ts`, you need to export the `onSearch` function, which will be called every time a search is performed. The input parameter is the search keyword, and you can customize the search interception logic here, for example:

```ts title="searchHook.ts"
