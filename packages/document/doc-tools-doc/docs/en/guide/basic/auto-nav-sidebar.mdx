...
When the type is an object, you can describe it as a file or a directory.
...
Among them, `name` means the file name, `with`/`without` suffix is ​​supported, `label` means the display name of the file in the sidebar, for example:
