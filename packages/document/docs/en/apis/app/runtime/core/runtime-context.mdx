...
- `initialData`: Initial data, can be used for SSR data injection
- `isBrowser`: Indicates whether the current runtime environment is a browser
- `routes`: Route configuration information
- `context`: Request context, containing `request` and `response` objects

### RequestContext

`RequestContext` is the type of the `context` property, containing request and response related information:
