  ...
  plugins: [
    ...
    i18nPlugin({
      localeDetection: {
        ...
        languages: ['zh', 'en'],
        fallbackLanguage: 'en',
        ignoreRedirectRoutes: ['/about', '/'], // Optional: routes that should not be redirected
      },
    }),
...
- `localePathRedirect`: Enable path-based locale redirection (e.g., `/zh/page`, `/en/page`)
- `languages`: Supported language codes
- `fallbackLanguage`: Default language when detection fails
- `ignoreRedirectRoutes`: Routes that should not be redirected (useful for special pages like about, home, etc.)
...
When using a shared instance, remote components will use the consumer's i18n instance by default when using `useTranslation()`. However, you can also directly use a specific i18next instance (e.g., `i18next.t()`) to access translations from that instance. When the main application switches languages, components using `useTranslation()` will automatically update.
...
const RemoteComponent = createLazyComponent({
  ...
  export: 'default',
  fallback: ({ error }) => {
    // fallback can be a function that receives error information
...
:::tip
The `fallback` parameter in `createLazyComponent` can be:

- A string: Simple loading text
- A React component: Static error boundary component
- A function: Receives error information `{ error }` and returns a React element, allowing for more granular error handling based on error types
  :::
