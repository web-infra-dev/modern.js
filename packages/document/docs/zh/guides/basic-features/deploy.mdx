...
全栈项目除了按照[纯前端项目](#纯前端项目-2)的方式配置 `edgeone.json` 外，需要注意：

1. 目前 Modern.js 还不支持在 EdgeOne Edge Functions 进行部署，我们将在后续的版本中支持。
...
全栈项目除了按照[纯前端项目](#纯前端项目-3)的方式配置 `wrangler.jsonc` 外，需要额外添加以下内容：
