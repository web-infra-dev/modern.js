...
配置完成后，你可以选择使用 EdgeOne CLI 进行部署：

* 在本地或全局安装 [EdgeOne CLI](https://edgeone.cloud.tencent.com/pages/document/162936923278893056)，并完成账号登录
* 在 `edgeone.json` 中配置 `name` 字段，在其中填写你的项目名称。
* 在项目根目录下运行 `edgeone pages deploy` 命令进行部署。

或者，你也可以在腾讯云控制台上创建项目：

* 方式一：平台部署
  * 打开腾讯云 EdgeOne 控制台，在 Pages 中，点击“创建项目”，选择“导入 Git 仓库”。
  * 按提示绑定账号，并选择你的仓库。
  * 构建配置、环境配置均保持默认。
  * 点击开始部署。
* 方式二：手动上传
  * 在本地使用 `MODERNJS_DEPLOY=edgeone modern deploy` 命令完成打包。
  * 进入项目下的 `.eo-output` 目录，将所有文件及文件夹压缩为 `.zip` 文件。
  * 打开腾讯云 EdgeOne 控制台，在 Pages 中，点击“创建项目”，选择“直接上传”。
  * 上传前述步骤中的压缩文件。
  * 点击开始部署。
...
全栈项目请按照[纯前端项目](#纯前端项目-2)的方式配置 `edgeone.json`。部署方式与纯前端项目完全相同。

需要注意：目前 Modern.js 还不支持在 EdgeOne Edge Functions 进行部署，我们将在后续的版本中支持。

在 BFF 和 Data Loader 中，均可以使用类似于获取 Cloudflare Workers Bindings 的方式获取腾讯云 EdgeOne 的 EventContext。

## 阿里云 ESA

在阿里云 ESA 上部署，通常需要配置 `esa.jsonc` 文件。
...
配置完成后，你可以选择使用 ESA CLI 进行部署：

* 在本地或全局安装 [ESA CLI](https://help.aliyun.com/zh/edge-security-acceleration/esa/user-guide/functions-and-pages-cli-tool)，并完成账号登录
* 在 `esa.jsonc` 中配置 `name` 字段，在其中填写你的项目名称。
* 在项目根目录下运行 `esa deploy` 命令进行部署。

或者，你也可以提交到 Git，在阿里云控制台上创建项目：

* 打开阿里云 ESA 控制台，在左侧菜单中选择 边缘计算 > 函数和Pages，点击“创建”。
* 选择“导入 Github 仓库”，按提示绑定 GitHub 账号，并选择你的仓库。
* 按需选择你的安装命令，构建命令和部署命令均保持默认。
* 点击开始部署。
...
* 目前仅支持 TypeScript、ES 项目的全栈项目部署至阿里云 ESA。
* 对全栈项目部署至阿里云 ESA 的支持尚处于实验阶段，我们暂时不保证其可用性；如果遇到问题，请通过 issue 反馈。
...
```json title="esa.jsonc"
{
  ...
  /* 上述配置与纯前端项目相同，保持不变，增加下面的配置 */
  "entry": "./.ali-esa/functions/handler.js"
}
```

配置完成后，部署步骤与[纯前端项目](#纯前端项目-3)相同。

## Cloudflare Workers

在 Cloudflare Workers 上部署，通常需要配置 `wrangler.jsonc` 文件。
...
配置完成后，你可以选择使用 Wrangler CLI 进行部署：

* 在本地或全局安装 [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/install-and-update/)
* 在 `wrangler.jsonc` 中配置 `name` 字段，在其中填写你的 Cloudflare Workers 项目名称。
* 在项目根目录下运行 `wrangler deploy` 命令进行部署。

或者，你也可以提交到 GitHub，在 Cloudflare 平台上创建项目：

* 打开 Cloudflare 仪表盘，在左侧菜单中选择 计算和AI > Workers 和 Pages，点击右上角“创建应用程序”。
* 选择 Continue with GitHub，按提示绑定 GitHub 账号，并选择你的仓库。
* 构建命令和部署命令均保持默认。
* 点击部署。
...
注意：目前仅支持 TypeScript、ES 项目的全栈项目部署至 Cloudflare Workers。
