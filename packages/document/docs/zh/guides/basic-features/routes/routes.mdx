...
如果你的项目通过 `--router tanstack` 创建，请改用 `@modern-js/runtime/tanstack-router`。
:::
...
1. 为什么要通过 `@modern-js/runtime/router` 或 `@modern-js/runtime/tanstack-router` 引入 API？

Modern.js 提供了按路由框架区分的运行时导出：

- React Router 模式使用 `@modern-js/runtime/router`。
- TanStack Router 模式使用 `@modern-js/runtime/tanstack-router`。
...
- `@modern-js/runtime/tanstack-router` 对应 TanStack Router 文档。
