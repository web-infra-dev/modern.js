...
`AGENTS.md` 是给 AI 编码助手的项目指引，告诉它在动手前先阅读随包文档、并遵循 Modern.js 的约定；`CLAUDE.md` 通过 `@AGENTS.md` 让 Claude Code 复用同一份指引。
...
当需要查阅随包文档之外的内容（例如其他框架版本，或随包未包含的章节）时，让 Agent 在线检索它即可，不必把文档复制进项目。
