...
Files that declare API routes under the [BFF Framework Mode](/guides/advanced-features/bff/type.html#framework-mode). Except for [convention files](/apis/app/hooks/api/api#white-list), files under the `lambda/` directory will be registered as the routes.

:::info
Using the `api` directory requires enabling the BFF function, and you need to execute the new command to enable the "BFF" function under the project.
