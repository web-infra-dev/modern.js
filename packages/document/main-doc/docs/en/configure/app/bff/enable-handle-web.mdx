---
title: enableHandleWeb
---

# bff.enableHandleWeb

- **Type:** `boolean`
- **Default:** `false`

:::caution
Please enable BFF functionality in the current application project root directory using [new](/apis/app/commands#modern-new) first.
:::

By default, the BFF service can only handle requests for BFF APIs.

When this value is set to `true`, page request traffic will also pass through BFF, and the default logic for page rendering built into Modern.js will run as the last middleware of the BFF service.

```ts title="modern.config.ts"
export default defineConfig({
  bff: {
    enableHandleWeb: true,
  },
});
```
