...
:::caution
Please enable BFF feature first by using 【[new](/apis/app/commands#modern-new)】 in the root directory of the current application project.
...
Unlike [dev.proxy](/configure/app/dev/proxy), the proxy in tshis section only applies to requests entering the BFF/API service; at the same time, this configuration can not only be used in the development environment, but also proxies corresponding requests in the production environment.
...
:::info
Under BFF proxy mode, if you don't need to write BFF interfaces, the API directory can be deleted; BFF proxy will still be enabled.
