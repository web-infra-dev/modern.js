...
Refer to [How to write plugins](/guides/topic-detail/framework-plugin/implement) for how to write custom plugins.
...
Modern.js has three types of built-in framework plugins:
...
When the plugin uses related fields that control the order, such as `pre` and `post`, the execution order will be adjusted based on the declared fields. Refer to [Relationship between plugins](/guides/topic-detail/framework-plugin/relationship) for more information.
...
To use plugins on npm, you need to install the plugins through a package manager and import them through `import`.
...
To use plugins from a local code repository, import them directly using a relative path.
