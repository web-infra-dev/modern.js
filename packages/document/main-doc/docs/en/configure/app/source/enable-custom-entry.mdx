...
This option is used for custom Modern.js entries. When this option is enabled, Modern.js will try to use the `src/entry.[jt]sx` file and `src/entry.server.[jt]s` as the project's entry point.
...
create `src/entry.server.ts` file，add custom behavior for rendering responses：
