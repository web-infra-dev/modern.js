...
Within the functional mode, the middleware writing protocols of different runtime frameworks may vary, while most other implementations remain consistent. Take Express as an example to introduce how to write a middleware by hand in the `api/_ app.ts` and add permission verification:
...
Finally, modify the front-end code located at `src/App.tsx` to invoke the login interface prior to accessing the `/api/hello` endpoint.
...
In the framework mode of Modern.js, middleware is not gathered in the `api/_app.ts` file, and the plugin itself controls the running process.
...
The route specified by the BFF function will be registered subsequent to the route specified by the `app.ts` file. Therefore, you have the ability to intercept the BFF function's route, preprocess it, and respond in advance.

:::caution
When operating in framework mode without an `app.ts`, Modern.js will automatically include the koa-body middleware. If there is an `app.ts`, developers who wish to use the BFF function with Body must verify that the koa-body middleware has been added.
