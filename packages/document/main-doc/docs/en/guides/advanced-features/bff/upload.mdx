...
BFF combined with [runtime framework](/guides/advanced-features/bff/frameworks.html) provides file upload capabilities, supporting integrated calls and pure function manual calls.
...
export const post = async (req: {formData: Record<string, any>}) => {
...
```
:::tip
In the BFF function, the client's uploaded file can be accessed through `formData` in the parameters. The value is an `Object`, with the key as the field name used during upload.
:::
