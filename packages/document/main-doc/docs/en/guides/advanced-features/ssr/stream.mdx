...
Modern.js provides a way to handle spider visits,
In Middleware or Custom Server hook add req.header `x-modern-spider-agent`, then let all of content load first for spider.
...
export const middleware = (ctx, next) => {
  const { req, res } = ctx;
  if (checkIsBot) {
    req.headers['x-modern-spider-agent'] = '1';
  }
