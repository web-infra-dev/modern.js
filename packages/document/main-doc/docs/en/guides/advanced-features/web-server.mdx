...
As a client-centric development framework, Modern.js has weak server-side customization capabilities. In some development scenarios, special server-level logic needs to be customized, such as user authentication, request preprocessing, and adding page rendering skeletons.
...
2. Functions such as marking and crawler optimization can be done in Middleware.
