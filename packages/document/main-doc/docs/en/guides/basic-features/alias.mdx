...
:::info
When enabling optional features, the `new` command will also dynamically add built-in alias. For example, when enabling BFF, the `@api` alias is added by default.
...
Modern.js also provides a way to customize alias. Taking adding the `@common` alias as an example:

For TypeScript projects, just configure `compilerOptions.paths` in the project's `tsconfig.json`:
...
JavaScript projects can configure [`source.alias`](/configure/app/source/alias) in `modern.config.js`:
