...
:::info
Modern.js v1 supports getting data through [useLoader](/guides/basic-features/data-fetch.html#useloader-(old-version)), which is no longer the recommended usage. We do not recommend mixing the two except during the migration process.
...
**`useLoader`** is an API in the old version of Modern.js. This API is a React Hook designed specifically for SSR applications, allowing developers to get data in components in isomorphic development.

:::tip
It is not necessary to use `useLoader` to get data in CSR projects.
:::
...
In this global variable, each piece of data is recorded, and during the browser-side rendering process, this data is used first. If the data does not exist, the `useLoader` function will be executed again.
