...
The `customKey` parameter is used to customize the cache key. It is a function that receives an object containing the parameters of the cached function, the function itself, and the serialized result of the parameters, and returns a string or Symbol type as the cache key. This is very useful in some scenarios, such as when the function reference changes (such as hot update or dynamic import), but you want to still return the cached data.
...
// You can utilize the generatedKey parameter
