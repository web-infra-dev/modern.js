...
In each routing component (`layout.tsx`, `page.tsx`, `$.tsx`), we can export a `shouldRevalidate` function that controls which routes are reloaded, and when this function returns true, the data for the corresponding route is reloaded.
