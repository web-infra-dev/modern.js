...
If you need to use environment variables with any other names in your code, you can configure [`source.globalVars`](/configure/app/source/global-vars), for example:
...
In addition to environment variables, Modern.js also supports replacing variables in code with other values or expressions, which can be used to distinguish between the development environment and the production environment in code.
