...
Modern.js early versions supported runtime configuration and initialization operations through `config` function and `init` function exported in route layout files. These methods are still **supported**, but we **strongly recommend** using the new methods.
...
We strongly recommend using Runtime configuration files to implement Runtime dynamic configuration instead of using the `config` function. For more details, please refer to [Runtime Configuration](/configure/app/runtime/0-intro).
...
We strongly recommend using [Runtime Plugins](/plugin/introduction.html#runtime-plugin) to implement rendering preprocessing logic instead of using the `init` function. For more details, please refer to [Render Preprocessing](/guides/basic-features/render/before-render).

:::

:::warning

In future versions, `config` function and `init` function will be gradually **deprecated**. We recommend migrating to Runtime configuration files and Runtime plugin solutions as soon as possible.
