...
Not all top-level directories under `src/` become project entries. The directory where the entry is located must meet one of the following three conditions:
...
customBootstrap(ModernRoot, () => render(<ModernRoot />, 'root'));
```
