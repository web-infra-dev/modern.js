...
Rsbuild provides an API that supports access to the Dev Server's HTTP capabilities. Based on this handler, Modern.js builds the Web Server for the development environment. It includes capabilities needed during development stages, such as HMR and Proxy, provided by Rsbuild, and routing and rendering capabilities provided by Modern.js:
...
You can run the [`modern build`](/apis/app/commands.html#modern-build) command to build the application and the [`modern serve`](/apis/app/commands.html#modern-serve) command to run the Web server, starting the Modern.js application.
...
Modern.js offers a standalone deployment solution. When running the [`modern deploy`](/apis/app/commands.html#modern-deploy) command, the artifacts will include an entry file for running the Web server.