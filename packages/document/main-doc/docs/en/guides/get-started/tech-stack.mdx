...
Modern.js uses [Babel](https://babeljs.io/), [SWC](https://swc.rs/), or [esbuild](https://esbuild.github.io/) as JavaScript transpilation tools to transpile TypeScript or JSX into JavaScript code that can run in browsers and perform syntax downgrades.

- When using Webpack for bundling, the default tool is Babel, which can be switched to SWC or esbuild.
- When using Rspack for bundling, the default tool is SWC, and switching to other tools is not currently supported.
