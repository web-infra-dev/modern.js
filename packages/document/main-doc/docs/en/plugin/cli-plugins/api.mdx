...
| `internalDirectory`  | `string`             | 框架内部临时输出目录的绝对路径                                        | -                                   |
...
| `internalDirAlias`   | `string`             | 框架内部临时输出目录的别名                                            | -                                   |
| `internalSrcAlias`   | `string`             | 框架 `src` 目录的别名                                                 | -                                   |
...
注册一个函数来使用 chain API 修改 webpack 或者 Rspack 配置。
...
注册一个函数来修改 Rspack 的配置。(当使用 Rspack 作为打包工具时)
...
注册一个函数来使用 [webpack-chain](https://github.com/neutrinojs/webpack-chain) 修改 Webpack 配置。(当使用 Webpack 作为打包工具时)
...
注册一个函数来直接修改 Webpack 配置对象。(当使用 Webpack 作为打包工具时)
...
-   **类型:**  `api.deploy(deployFn: () => void | Promise<void>)`
-   **参数:**
    -   `deployFn`: 部署时执行的任务。
