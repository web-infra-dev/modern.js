...
Install the tailwindcss package to integrate Tailwind CSS:

import { PackageManagerTabs } from '@theme';

<PackageManagerTabs
  command={{
    npm: 'npm install tailwindcss@^3 -D',
    yarn: 'yarn add tailwindcss@^3 -D',
    pnpm: 'pnpm install tailwindcss@^3 -D',
  }}
/>

Register the tailwindcss PostCSS plugin via `postcss.config.cjs`:
...
Create a tailwind.config.js file in the root directory of the current project and add the following content:
...
Add the @tailwind directives in the CSS entry file:
