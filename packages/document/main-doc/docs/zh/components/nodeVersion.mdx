在开始使用前，你需要安装 [Node.js](https://nodejs.org/)，并保证 Node.js 版本不低于 18.20.8，**我们推荐使用 Node.js 20 的 LTS 版本**。
...
此外，在安装 nvm 或 fnm 后，然后只要仓库根目录下有内容为 `lts/iron` 的 `.nvmrc` 文件，进入这个仓库时就会自动安装或切换到正确的 Node.js 版本。

:::warning
Modern.js 目前仍支持 Node 16，但会在 2025 年 10 月正式终止对 Node.js 16 的支持，为确保您的项目顺利过渡，请在 2025 年 10 月前完成 Node.js 版本升级。
:::
