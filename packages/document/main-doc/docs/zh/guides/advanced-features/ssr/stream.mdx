...
然而，当一个爬虫访问该页面时，它可能需要先加载所有内容，然后直接输出整个 HTML，而不是渐进式地加载它。

Modern.js 提供了一种方式来处理爬虫访问情况：

在 Middleware 或者自定义 Server hook 中添加请求头 `x-modern-spider-agent` 即可对爬虫的访问直接输出整个 HTML. 以下面代码为例
