...
当无 `options` 参数传入时，主要可以用于 SSR 项目，如可以在多个 data loader 中调用同一个 cachedFn 时，不会重复执行 cachedFn 函数。这样可以在不同的 data loader 中共享数据，同时避免重复的请求。EdenX 会在每次收到服务端请求时，重新执行 `fn` 函数(即缓存的生命周期是单次 ssr 渲染的请求)。
