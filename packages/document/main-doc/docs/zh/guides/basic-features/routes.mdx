...
Modern.js 早期版本支持在路由 layout 文件中通过导出 `config` 函数和 `init` 函数进行运行时配置和执行初始化操作。这些方式目前仍然**被支持**，但我们**强烈推荐**使用新方式。
...
我们强烈推荐使用 Runtime 配置文件来实现 Runtime 动态配置，而不是使用 `config` 函数。详情请参考 [Runtime 配置](/configure/app/runtime/0-intro)。
...
我们强烈推荐使用 [Runtime 插件](/plugin/introduction.html#runtime-插件) 来实现渲染预处理逻辑，而不是使用 `init` 函数。详情请参考 [渲染预处理](/guides/basic-features/render/before-render)。

:::

:::warning

未来版本中，`config` 函数和 `init` 函数将逐步**废弃**。建议尽早迁移到 Runtime 配置文件和 Runtime插件方案。
