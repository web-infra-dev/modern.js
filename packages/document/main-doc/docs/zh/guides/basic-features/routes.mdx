...
React Router v7 相比 React Router v6 breack change 非常少，同时 Modern.js 也对两个版本做了兼容，只需在项目中安装并注册相应的插件即可无缝升级。
...
react router v7 同样废弃了 `json` API，可以使用 `Response.json` 代替：
