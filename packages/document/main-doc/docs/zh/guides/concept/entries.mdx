...
在 Modern.js 应用中，你可以很方便的将单入口切换成多入口，直接在终端执行 `pnpm run new`，根据提示创建入口即可：
...
构建模式指的是不使用 Modern.js 提供的 Runtime 能力，只使用 Modern.js 构建能力的应用开发模式。当应用中未安装 `@modern-js/runtime` 依赖时，Modern.js 会认为当前应用为构建模式应用。
...
使用构建模式时，应用**将无法使用 Modern.js 框架的运行时能力**，比如：
