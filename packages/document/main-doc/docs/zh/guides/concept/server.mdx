...
Rsbuild 提供了 API 支持获取 Dev Server 的 HTTP，Modern.js 基于这个处理函数搭建了开发环境的 Web Server。它由 Rsbuild 提供包含 HMR、Proxy 等开发阶段需要的能力，由 Modern.js 提供了路由、渲染等能力：
...
你可以执行 [`modern build`](/apis/app/commands.html#modern-build) 来构建应用，执行 [`modern serve`](/apis/app/commands.html#modern-serve) 命令来运行 Web 服务器，启动 Modern.js 应用。

## 在生产环境中运行

在部署到生产环境时，产物体积应该尽可能小，而上述在 CI 中运行的方案，会保留原项目的所有产物。因此在生产环境，不推荐通过上述命令运行应用。

Modern.js 提供了独立的部署方案，当运行 [`modern deploy`](/apis/app/commands.html#modern-deploy) 命令时，产物中会包含可运行 Web 服务器的入口文件。