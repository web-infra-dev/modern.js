...
Modern.js 拥有一个强大的框架插件系统，允许开发者通过插件来扩展和增强框架的功能。这一系统不仅使得 Modern.js 本身具备灵活的架构能力，还为开发者提供了丰富的工具和接口，以便于实现自定义的功能和优化应用性能。
...
CLI 插件用于在应用执行 `modern` 命令时，提供额外的功能。例如，添加命令、修改配置、监听文件等。大多数构建相关的能力都可以通过 CLI 插件来实现。
...
Runtime 插件用于在应用运行 React 代码时，提供额外的功能。例如，执行初始化行为，实现 React 高阶组件（HOC）封装等。

Runtime 插件通过 `src/modern.runtime.ts` 中的 `plugins` 字段进行配置。
...
Server 插件用于在应用接受请求时，提供额外的功能。例如，添加中间件、修改请求响应等。
