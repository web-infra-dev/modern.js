...
在 Modern.js 中，目前可以直接使用两种插件，分别是 Modern.js 框架插件和 Rsbuild 构建插件。

## Modern.js 框架插件

Modern.js 拥有一个强大的框架插件系统。开发者可以通过插件来扩展和增强框架的功能。该系统不仅使 Modern.js 保持灵活的架构能力，还为开发者提供了丰富的工具和接口，帮助实现自定义功能和优化应用性能。
...
如果你需要开发 Modern.js 框架插件，请阅读 [Modern.js 插件系统](/plugin/plugin-system/introduction) 获取更多信息。
