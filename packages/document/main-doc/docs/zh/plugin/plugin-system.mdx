...
**CLI 插件**： - **作用阶段**：构建时（执行 `modern` 命令时）。 - **典型场景**： - 扩展命令行工具。 - 修改构建配置。 - 监听文件变化。 - 控制构建流程。 - **配置方式**：`modern.config.ts` 中的 `plugins` 字段。

**Runtime 插件**： - **作用阶段**：应用运行时（浏览器/Node.js 环境）。 - **典型场景**： - 初始化全局状态或服务。 - 封装 React 高阶组件（HOC）。 - 拦截或修改路由行为。 - 控制渲染流程。 - **配置方式**：`src/modern.runtime.ts` 中的 `plugins` 字段。

{/_ **Server 插件**： - **作用阶段**：服务端请求处理阶段。 - **典型场景**： - 添加自定义中间件。 - 扩展服务端 API。 - 定制化 SSR 逻辑。 - 与后端框架集成。 - **配置方式**：`server/modern.server.ts` 中的 `plugins` 字段。 _/}
