...
Suffixes for js files and type description files in automation products based on [format](#format) and [type](https://nodejs.org/api/packages.html#type).

- **Type**: `boolean`
- **Default**: `false`
- **Version**: `2.38.0`

When off, js products are suffixed with `.js` and type description files are suffixed with `d.ts`.

When on, node loads `.js` as esm by default when type is `module`, so when we want to output cjs products, the js product is suffixed with `.cjs` and the type description file is suffixed with `d.cts`.
...
But [noUselessIndex](https://github.com/import-js/eslint-plugin-import/blob/main/docs/rules/no-useless-path-segments.md#nouselessindex) will break this behavior.
...
When building CommonJS/ESM artifacts, inject some filler code such as `__dirname` which can only be used in CommonJS.
After Enable this option, it will compiler `__dirname` as `path.dirname(fileURLToPath(import.meta.url))` when format is esm.
