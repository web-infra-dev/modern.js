...
根据 [format](#format) 和 [type](https://nodejs.org/api/packages.html#type) 自动化产物里 js 文件和类型描述文件的后缀。
...
但 [noUselessIndex](https://github.com/import-js/eslint-plugin-import/blob/main/docs/rules/no-useless-path-segments.md#nouselessindex) 会破坏此行为，
