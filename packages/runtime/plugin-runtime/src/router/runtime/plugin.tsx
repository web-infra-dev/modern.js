...
import { createRouterFromPayload } from './RSCHydratedRouter';
import {
...
function useRouterCreation(
  ...
  return useMemo(() => {
    if (rscPayload) {
      try {
        const router = createRouterFromPayload(rscPayload);
    ...
    const modifiedRoutes = hooks.modifyRoutes.call(routeObjects);
