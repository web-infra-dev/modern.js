...
function getImportSource(importStatement: string) {
  return importStatement.match(/from\s+['"]([^'"]+)['"];?/)?.[1] || '';
}

function sortImportStatements(importStatements: string[]) {
  return [...importStatements].sort((a, b) => {
    const sourceComparison = getImportSource(a).localeCompare(
      getImportSource(b),
    );

    return sourceComparison || a.localeCompare(b);
  });
}
...
export async function generateTanstackRouterTypesSourceForEntry(opts: {
  ...
  const getImportNamesForLoader = async (
    ...
    const actionName = hasAction
      ...
      : null;
    if (inline) {
      const specifiers = actionName
        ? [`action as ${actionName}`, `loader as ${importName}`]
        : [`loader as ${importName}`];
      imports.push(
        specifiers.length > 1
          ? `import {\n  ${specifiers.join(',\n  ')},\n} from ${quote(relImport)};`
          : `import { ${specifiers[0]} } from ${quote(relImport)};`,
      );
    } else {
  ...
  const rootStaticDataSnippet = createRouteStaticDataSnippet({
    modernRouteId: (rootModern as any)?.id as string | undefined,
    loaderName: rootLoaderName,
    actionName: rootActionName,
  });
  if (rootStaticDataSnippet) {
    rootOpts.push(rootStaticDataSnippet);
  }
  const rootRouteOptionsSource = rootOpts.length
    ? `{\n  ${rootOpts.join('\n  ')}\n}`
    : '{}';

  const routeTreeChildren = topLevelVars.length
    ? `[
  ${topLevelVars.join(',\n  ')},
]`
    : '[]';
...
export const rootRoute = createRootRouteWithContext<ModernRouterContext>()(${rootRouteOptionsSource});

${statements.join('\n\n')}

export const routeTree = rootRoute.addChildren(${routeTreeChildren});
