...
export class Server implements ServerCore {
  ...
  private async prepareAPIHandler() {
    ...
    return this.core.all(prefix, httpCallBack2HonoMid(middleware));
  }
  ...
  public get(path: string, middleware: Middleware) {
    this.core.get(path, middleware);
  }

  public post(path: string, middleware: Middleware) {
    this.core.post(path, middleware);
  }
  ...
  public handle(
    ...
    return this.core.handle(request, ...args);
  }
}
