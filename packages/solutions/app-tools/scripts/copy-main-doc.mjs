...
// `main-doc/`, so every project gets version-matched offline docs
...
// Flattened bundle root: docs land directly under `main-doc/` (no `docs/en`
// nesting) to keep the path agents read as short as possible.
const target = path.resolve(pkgRoot, 'main-doc');
...
console.log(`[copy-main-doc] bundled ${count} doc files into main-doc`);
