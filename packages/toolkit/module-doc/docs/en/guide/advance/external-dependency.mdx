...
In module projects, **by default, third-party dependencies under `"dependencies"` and `"peerDependencies"` are not packaged**. This is because when the npm package is released, the dep for the npm package will be installed when the npm package is installed.
