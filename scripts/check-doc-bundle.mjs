...
const bundled = packJson[0].files.filter(f =>
  f.path.startsWith('main-doc/docs/en/'),
);
...
const resolvesInBundle = spec => {
  const rel = spec.replace('@site-docs-en/', 'main-doc/docs/en/');
  return [
    '',
    '.mdx',
    '.md',
...
for (const entry of sourceFiles) {
  ...
  for (const match of content.matchAll(importRegex)) {
    ...
    if (spec.startsWith('@site-docs-en/')) {
      ...
      const resolved = path
        .join('main-doc/docs/en', relDir, spec)
        .replaceAll(path.sep, '/');
      if (
        !resolvesInBundle(
          resolved.replace('main-doc/docs/en/', '@site-docs-en/'),
        )
      ) {
        console.error(
