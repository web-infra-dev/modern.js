...
test('should compile CSS modules correctly', async () => {
  ...
  if (builder.providerType === 'rspack') {
    expect(content).toEqual(
      '.the-a-class{color:red}.the-b-class-_1c322{color:blue}.the-c-class-bc0a68{color:#ff0}.the-d-class{color:green}',
    );
  } else {
    expect(content).toEqual(
...
test('should treat normal CSS as CSS modules when disableCssModuleExtension is true', async () => {
  ...
  if (builder.providerType === 'rspack') {
    expect(content).toEqual(
      '.the-a-class-addade{color:red}.the-b-class-_1c322{color:blue}.the-c-class-bc0a68{color:#ff0}.the-d-class{color:green}',
    );
  } else {
    expect(content).toEqual(
...
test('should compile CSS modules follow by output.cssModules', async () => {
  ...
  if (builder.providerType === 'rspack') {
    expect(content).toEqual(
      '.the-a-class{color:red}.the-b-class-_1c322{color:blue}.the-c-class{color:#ff0}.the-d-class{color:green}',
    );
  } else {
    expect(content).toEqual(
