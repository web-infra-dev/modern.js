...
import CreateMwaWindows from './assets/create-mwa-windows.mp4';

<video controls width="100%" src={CreateMwaWindows} type="video/mp4"></video>

在项目目录执行 `pnpm run dev` 命令可进行项目开发和调试：

import MwaDevWindows from './assets/mwa-dev-windows.mp4';

<video controls width="100%" src={MwaDevWindows} type="video/mp4" ></video>

在项目目录执行 `pnpm run build` 命令可进行项目打包：

import MwaBuildWindows from './assets/mwa-build-windows.mp4';

<video controls width="100%" src={MwaBuildWindows} type="video/mp4" ></video>

在项目目录执行 `pnpm run new` 命令可开启项目扩展功能，如开启 BFF：

import MwaNewWindows from './assets/mwa-new-windows.mp4';

<video controls width="100%" src={MwaNewWindows} type="video/mp4" ></video>

在项目目录执行 `pnpm run test` 命令，可运行单元测试和集成测试：

import MwaTestWindows from './assets/mwa-test-windows.mp4';

<video controls width="100%" src={MwaTestWindows} type="video/mp4" ></video>

### 示例：Monorepo 工程

在 Windows 平台上，可使用 `npx @modern-js/create` 创建项目组：

import CreateMonorepoWindows from './assets/create-monorepo-windows.mp4';

<video controls width="100%" src={CreateMonorepoWindows} type="video/mp4" ></video>

在项目组中执行 `pnpm run new` 可创建子项目，例如创建模块子项目：

import MonorepoCreateModuleWindows from './assets/monorepo-create-module-windows.mp4';

<video controls width="100%" src={MonorepoCreateModuleWindows} type="video/mp4" ></video>

在模块子项目目录执行 `pnpm run new` 可开启模块扩展功能，例如 Storybook：

import MonorepoCreateStorybookWindows from './assets/monorepo-module-storybook-windows.mp4';

<video controls width="100%" src={MonorepoCreateStorybookWindows} type="video/mp4" ></video>

另外，Modern.js 官网上，针对 Windows 和 macOS 上不同的命令，将会使用以下形式展示：

![Windows 命令文档展示形式](./assets/doc-windows.png)
...
import EnableDeploy from './assets/enable-deploy.mp4';

<video controls width="100%" src={EnableDeploy} type="video/mp4" ></video>

使用阿里云 FC + 部署应用：

import DeployAliyun from './assets/deploy-aliyun.mp4';

<video controls width="100%" src={DeployAliyun} type="video/mp4" ></video>

> [了解更多](https://modernjs.dev/docs/guides/features/server-side/deploy/modern-server/use-lambda/aliyun-fc)

使用腾讯云 SCF + COS 部署应用：

import DeployTencent from './assets/deploy-tencent.mp4';

<video controls width="100%" src={DeployTencent} type="video/mp4" ></video>

> [了解更多](https://modernjs.dev/docs/guides/features/server-side/deploy/modern-server/use-lambda/tencent-scf)

使用静态 Web 服务器托管产物：

import DeployStatic from './assets/deploy-static.mp4';

<video controls width="100%" src={DeployStatic} type="video/mp4" ></video>
