...
For example, there are two files `index.ts` and `index.module.scss` under a certain folder:
...
After executing the build, the `index.module.scss.d.ts` type declaration file will be automatically generated:
...
Then open the `index.ts` file again, you will see that the `styles` object already has a exact type.
